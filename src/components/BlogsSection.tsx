    ...
    <section className="relative w-full overflow-hidden">
      ...
      <div className="relative max-w-[1440px] mx-auto text-center text-white px-4 pt-8 pb-16 md:pt-10 md:pb-28" style={{ zIndex: 1 }}>
        ...
        <div className="flex flex-col md:flex-row justify-center gap-5 md:gap-[30px]">
          {blogs.map((post) => (
            <div key={post.id} className="w-full md:w-[380px] bg-white rounded-[20px] md:rounded-[24px] overflow-hidden shadow-md">
              <img src={getImageUrl(post.banner)} alt={post.title} className="w-full h-[180px] md:h-[214px] object-cover" />
              <div className="p-4 md:p-5 flex flex-col gap-2 md:gap-3 text-left">
                ...
                <h3 className="text-[16px] md:text-[18px] font-semibold text-[#121212] leading-[24px] md:leading-[28px]">{post.title}</h3>
                <div className="border-t border-[#DEDEDE]" />
                <p className="text-[14px] md:text-[16px] text-[#AAAAAA] leading-[22px] md:leading-[26px] line-clamp-2">{post.shortDescription}</p>
                <Link to={`/blog/${post.slug}`} className="flex justify-center items-center gap-2 bg-[#CEF17B] text-[#064734] font-semibold py-2.5 md:py-3 rounded-[43px] text-[14px] md:text-[16px]">
